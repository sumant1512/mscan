...
export class FeaturesService {
  ...
  createFeature(data: CreateFeatureRequest): Observable<ApiResponse<{ feature: Feature }>> {
    return this.http.post<ApiResponse<{ feature: Feature }>>(
      this.apiUrl,
      data
    );
  }
   ...
   * Requires: SUPER_ADMIN role
   */
  updateFeature(id: string, data: UpdateFeatureRequest): Observable<ApiResponse<{ feature: Feature }>> {
    return this.http.put<ApiResponse<{ feature: Feature }>>(
      `${this.apiUrl}/${id}`,
      data
    );
  }
   ...
   * Requires: SUPER_ADMIN role
   */
  enableFeatureForTenant(tenantId: string, featureId: string): Observable<ApiResponse<{ tenant_feature: any }>> {
   ...
   */
  checkFeatureForTenant(tenantId: string, featureCode: string): Observable<ApiResponse<{ enabled: boolean }>> {
