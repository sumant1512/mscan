...
exports.createDealer = asyncHandler(async (req, res) => {
  ...
  validateRequiredFields(req.body, ['full_name', 'email', 'phone_e164', 'shop_name', 'address', 'pincode', 'city', 'state']);
...
exports.listDealers = asyncHandler(async (req, res) => {
  ...
  const { dealers, total } = await dealerService.listDealers(tenantId, { page, limit, search });
