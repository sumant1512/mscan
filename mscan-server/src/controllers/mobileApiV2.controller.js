...
exports.getProducts = async (req, res) => {
  try {
    ...
    console.error('Mobile API v2 - Get products error:', error);
    await logApiUsage(req.apiAuth?.verificationAppId, 'mobile', req, 500);
    res.status(500).json({
      success: false,
...
exports.getProduct = async (req, res) => {
  try {
    ...
    console.error('Mobile API v2 - Get product error:', error);
    await logApiUsage(req.apiAuth?.verificationAppId, 'mobile', req, 500);
    res.status(500).json({
      success: false,
...
exports.getTemplates = async (req, res) => {
  try {
    ...
    console.error('Mobile API v2 - Get templates error:', error);
    await logApiUsage(req.apiAuth?.verificationAppId, 'mobile', req, 500);
    res.status(500).json({
      success: false,
 ...
 */
exports.getTemplateAttributes = async (req, res) => {
  try {
    ...
    if (result.rows.length === 0) {
      ...
      return res.status(404).json({
        success: false,
        error: 'Template not found or has no attributes'
      });
    }
    ...
    console.error('Mobile API v2 - Get template attributes error:', error);
    await logApiUsage(req.apiAuth?.verificationAppId, 'mobile', req, 500);
    res.status(500).json({
      success: false,
